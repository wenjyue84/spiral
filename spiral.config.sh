...
SPIRAL_PRD_STREAM_THRESHOLD_KB="2048 # streaming jq path has a bug; keep on in-memory path until prd.json > 2MB"  # streaming jq path has a bug; keep on in-memory path until prd.json > 2MB" # streaming jq path has a bug; keep on in-memory path until prd.json > 2MB"  # streaming jq path has a bug; keep on in-memory path until prd.json > 2MB"  # streaming jq path has a bug; keep on in-memory path until prd.json > 2MB
