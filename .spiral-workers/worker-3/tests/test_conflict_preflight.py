...
class TestPriorityRank:
    def test_critical(self):
        assert cp.priority_key(_story("US-1", priority="critical")) == 20

    def test_high(self):
        assert cp.priority_key(_story("US-1", priority="high")) == 40

    def test_medium(self):
        assert cp.priority_key(_story("US-1", priority="medium")) == 60

    def test_low(self):
        assert cp.priority_key(_story("US-1", priority="low")) == 80

    def test_unknown_defaults_medium(self):
        assert cp.priority_key({"id": "US-1"}) == 60
