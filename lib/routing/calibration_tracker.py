    ...
    return {
        ...
        "recent_window": {
            ...
            "by_tier": {
                ...
                for tier, durations in defaultdict(
                    list, {r["estimated_complexity"]: [r["actual_duration_s"]] for r in recent_records}
                ).items()
            },
        },
...
def main() -> None:
    ...
    record_parser.add_argument(
        "--passed", type=lambda x: x.lower() == "true", required=True, help="Whether story passed"
    )
    ...
    if args.command == "record":
        ...
        print(
            f"✓ Recorded calibration: {args.story_id} ({args.estimated_complexity}): {args.actual_duration_s}s",
            file=sys.stderr,
        )
