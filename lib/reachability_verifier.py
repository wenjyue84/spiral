...
def verify_reachability(story_title: str) -> dict:
    ...
    module_names = [Path(f).stem for f in new_files if f.startswith("lib/") and f.endswith(".py")]
    ...
    return {"reachable": reachable, "modules_checked": module_names, "call_sites": call_sites}
