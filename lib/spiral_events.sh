...
#
# Worker isolation: When SPIRAL_WORKER_ID is set, events are written to
# .spiral/events/worker-N.jsonl instead of the shared spiral_events.jsonl.
# This prevents concurrent append corruption on Windows (no PIPE_BUF
# guarantee). Worker event files are merged at iteration end by
# lib/observability/merge_events.py (mirrors results.tsv worker isolation).
#
# Usage:
...
# ── Helper: append a structured JSONL event ──────────────────────────────
...
log_spiral_event() {
  ...
  # Worker isolation: per-worker event file to avoid concurrent append
  # corruption on Windows (no PIPE_BUF guarantee with 3+ workers).
  if [[ -n "${SPIRAL_WORKER_ID:-}" ]]; then
    local events_dir="${SCRATCH_DIR:-/tmp}/events"
    mkdir -p "$events_dir" 2>/dev/null || true
    log_file="$events_dir/worker-${SPIRAL_WORKER_ID}.jsonl"
  else
    log_file="${SCRATCH_DIR:-/tmp}/spiral_events.jsonl"
  fi

  # Include current SPIRAL_LOG_LEVEL in every entry (US-130)
  ...
  # Add worker_id field when running as a worker
  local worker_field=""
  if [[ -n "${SPIRAL_WORKER_ID:-}" ]]; then
    worker_field=",\"worker_id\":${SPIRAL_WORKER_ID}"
  fi
  if [[ -n "$extra" ]]; then
    line="{\"ts\":\"$ts\",\"event\":\"$event\",\"run_id\":\"${SPIRAL_RUN_ID:-}\",$level_field,$extra$worker_field$trace_fields}"
  else
    line="{\"ts\":\"$ts\",\"event\":\"$event\",\"run_id\":\"${SPIRAL_RUN_ID:-}\",$level_field$worker_field$trace_fields}"
  fi
  ...
  if [[ -z "${SPIRAL_WORKER_ID:-}" \
    ...
    if [[ "$count" -gt "${SPIRAL_EVENT_LOG_MAX_LINES:-10000}" ]]; then
      mv "$log_file" \
        "${log_file%.jsonl}-$(date -u +%Y%m%d-%H%M%S).jsonl" \
        2>/dev/null || true
    fi
  fi
}
