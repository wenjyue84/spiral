...
def find_similar_passed_stories(story: dict[str, Any], prd: dict[str, Any], top_k: int = 3) -> list[tuple[str, float]]:
    """
    Find top-k most semantically similar stories that have passed.
    Uses difflib.SequenceMatcher on concatenated title+description.
    Returns list of (story_id, similarity_score) tuples, sorted by score descending.
    """
    if not story or not prd:
        return []

    query = f"{story.get('title', '')} {story.get('description', '')}".lower()
    if not query.strip():
        return []

    all_stories = prd.get("userStories", [])
    passed_stories = [s for s in all_stories if s.get("passes") is True and s.get("id") != story.get("id")]

    similarities: list[tuple[str, float]] = []
    for candidate in passed_stories:
        ...
        similarities.append((candidate.get("id", ""), ratio))

    similarities.sort(key=lambda x: x[1], reverse=True)
    return similarities[:top_k]


def get_git_diff_summary(story_id: str, max_chars: int = 8000, lines_per_file: int = 60) -> str:
    """
    Retrieve git diff for a story using git log --grep=<story_id> -p.
    Truncates to lines_per_file per file and caps total at max_chars.
    Returns summary string, empty if no commits found.
    """
    try:
        result = subprocess.run(
            ["git", "log", "--all", "--grep=" + story_id, "-p", "--no-merges"],
            capture_output=True,
            text=True,
            timeout=10,
            encoding="utf-8",
...
def _inject_reference_implementations(story: dict[str, Any], prd: dict[str, Any]) -> dict[str, Any]:
    """
    Find top-3 similar passed stories and inject them as reference implementations.
    Adds _reference_implementations field with list of {id, title, score, diff_summary}.
    """
    similar = find_similar_passed_stories(story, prd, top_k=3)
    if not similar:
        return story

    all_stories = {s.get("id"): s for s in prd.get("userStories", [])}
    reference_impls: list[dict[str, Any]] = []
    total_chars = 0
    max_total_chars = 8000

    for story_id, score in similar:
        if total_chars > max_total_chars:
            break

        similar_story = all_stories.get(story_id)
        if not similar_story:
            continue

        diff_summary = get_git_diff_summary(story_id, max_chars=max_total_chars - total_chars)
        total_chars += len(diff_summary)

        reference_impls.append(
            {
                "id": story_id,
                "title": similar_story.get("title", ""),
                "score": round(score, 2),
    ...
    if reference_impls:
        story["_reference_implementations"] = reference_impls
...
def enrich_stories(
    ...
    # Load full prd.json to find similar passed stories for reference implementations
    prd_data: dict[str, Any] = {}
    try:
        prd_path = os.path.join(os.path.dirname(validated_path), "..", "..", "prd.json")
        ...
        if batch_result:
            ...
            for relative_idx, enriched_list in batch_result.items():
                if relative_idx < len(batch_indices):
                    ...
                    if prd_data:
                        enriched_list = [_inject_reference_implementations(s, prd_data) for s in enriched_list]
            ...
            for sorted_idx, story in batch_eligible:
                ...
                if prd_data:
                    result = [_inject_reference_implementations(s, prd_data) for s in result]
    ...
    for i, story in enumerate(sorted_stories):
        if i not in processed_indices and not any(idx == i for idx, _ in eligible_stories):
            ...
            if prd_data:
                story = _inject_reference_implementations(story, prd_data)
