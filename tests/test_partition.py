...
class TestAssignStories:
    ...
    def test_rebalance_no_worker_starved(self):
        ...
        stories = [{"id": f"US-{i}", "priority": "medium", "filesTouch": ["shared.py"]} for i in range(6)]
    ...
    def test_rebalance_respects_max_per_worker(self):
        ...
        stories = [{"id": f"US-{i}", "priority": "medium", "filesTouch": ["shared.py"]} for i in range(7)]
        n_workers = 3
        buckets = assign_stories(stories, n_workers, use_complexity=False)
        max_allowed = math.ceil(len(stories) / n_workers)
        for i, b in enumerate(buckets):
            assert len(b) <= max_allowed, f"Worker {i} has {len(b)} stories, max allowed is {max_allowed}"
