    ...
    async def test_filter_controls_are_visible(self) -> None:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                assert failure_value == "all", (
                    f"Failure Type filter should default to 'all', got {failure_value}"
                )

                model_value = await model_filter.get_attribute("value")
                assert model_value == "all", (
                    f"Model Tier filter should default to 'all', got {model_value}"
                )

                outcome_value = await outcome_filter.get_attribute("value")
                assert outcome_value == "all", (
                    f"Outcome filter should default to 'all', got {outcome_value}"
                )
            ...
            try:
                ...
                if failure_filter:
                    ...
                    assert len(options) > 0, (
                        "Failure Type filter should have at least 'all' option"
                    )
                ...
                if model_filter:
                    ...
                    assert len(options) > 0, (
                        "Model Tier filter should have at least 'all' option"
                    )
                ...
                if outcome_filter:
                    ...
                    assert len(options) > 0, (
                        "Outcome filter should have at least 'all' option"
                    )
    ...
    async def test_outcome_filter_works(self) -> None:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                table_rows_initial = await page.query_selector_all(
                    "table tbody tr"
                )
                ...
                table_rows_filtered = await page.query_selector_all(
                    "table tbody tr"
                )
                ...
                if filtered_count == 0:
                    ...
                    assert empty_state is not None, (
                        "Should show empty state when filter results in no rows"
                    )
                    logger.info("✓ Empty state displayed when no rows match filter")
                else:
                    logger.info(
                        f"✓ Filter reduced rows from {initial_count} to {filtered_count}"
                    )
                ...
                table_rows_reset = await page.query_selector_all(
                    "table tbody tr"
                )
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if len(table_rows) == 0:
                    ...
                    assert empty_state is not None, (
                        "Should show empty state when no rows match"
                    )
    ...
    async def test_sort_toggle_works_with_filters(self) -> None:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # Find sort buttons
                recent_btn = await page.query_selector(
                    "[data-testid='sort-recent-btn']"
                )
                count_btn = await page.query_selector(
                ...
                assert "violet-100" in count_class, (
                    "Most Retried button should be highlighted after click"
                )
                ...
                assert "violet-100" in recent_class_after, (
                    "Most Recent button should be highlighted after click"
                )
                    ...
                    if value and value != "all":
                        ...
                        if empty_state is not None:
                            ...
                            assert "filter" in text.lower(), (
                                "Empty state message should mention filters"
                            )
                            logger.info(
                                f"✓ Empty state message displayed: {text}"
                            )
                            return
