...
from lib.results_tsv import HEADER, ResultsRecord, write_results_tsv
...
class TestMetricLoading:
    ...
    def test_basic_aggregation(self) -> None:
        records = [
            ...
            _make_record(spiral_iter=1, status="reject", duration_sec="200",
...
class TestTimeoutScaling:
    def test_increase_on_high_timeout_rate(self) -> None:
        ...
        for i in [1, 2]:
            ...
            for _ in range(4):
                records.append(_make_record(spiral_iter=i, status="error",
                                           failure_root_cause="timeout"))
            for _ in range(6):
                records.append(_make_record(spiral_iter=i, status="pass"))

        tuner = _make_tuner(records, current_iter=2, env_overrides={
...
class TestDiffLimitScaling:
        ...
        for i in [1, 2]:
            ...
            for _ in range(3):
                records.append(_make_record(
                    spiral_iter=i, status="reject",
                    failure_root_cause="oversized_diff",
                ))
            for _ in range(5):
                records.append(_make_record(spiral_iter=i, status="reject",
                                           failure_root_cause="type_error"))
    ...
    def test_decrease_after_3_clean_iters(self) -> None:
        ...
        tuner = _make_tuner(records, current_iter=3, env_overrides={
...
class TestCooldown:
    def test_skips_recently_adjusted(self) -> None:
        ...
        tuner = _make_tuner(records, current_iter=2, history=history, env_overrides={
            "SPIRAL_MAX_DIFF_LINES": "800",
        })
        adj = tuner._rule_diff_limit(tuner._recent_metrics(3))
    ...
    def test_allows_after_cooldown(self) -> None:
        ...
        tuner = _make_tuner(records, current_iter=4, history=history, env_overrides={
            "SPIRAL_MAX_DIFF_LINES": "800",
        })
        adj = tuner._rule_diff_limit(tuner._recent_metrics(3))
...
class TestPersistence:
    def test_persist_appends_to_file(self) -> None:
        ...
        tuner.persist([
            TuningAdjustment("SPIRAL_MAX_DIFF_LINES", "800", "1000",
                             "diff_limit_scaling", "test reason"),
        ])
