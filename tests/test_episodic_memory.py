...
from episodic_memory import init_db, list_recent, query_similar, write_memory
...
class TestWriteAndQuery:
    def test_write_then_query_returns_result(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            results = query_similar("episodic memory SQLite", top_k=3, db_path=db_path)
    ...
    def test_query_ranks_relevant_higher(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = os.path.join(td, "test.db")
            write_memory("US-1", "Add logging framework", "Added winston logger", "pass", 1, db_path)
            write_memory("US-2", "Fix database migration", "Fixed alembic upgrade", "fail", 2, db_path)
            write_memory("US-3", "Add structured logging", "JSON log output", "pass", 3, db_path)
            results = query_similar("logging framework structured", top_k=3, db_path=db_path)
...
class TestListRecent:
    def test_list_returns_newest_first(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = os.path.join(td, "test.db")
            write_memory("US-10", "first story", "approach A", "pass", 1, db_path)
            write_memory("US-11", "second story", "approach B", "fail", 2, db_path)
    ...
    def test_list_respects_limit(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = os.path.join(td, "test.db")
            for i in range(5):
                write_memory(f"US-{i}", f"story {i}", f"approach {i}", "pass", i, db_path)
...
class TestEmptyQuery:
    def test_query_empty_db_returns_empty(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            db_path = os.path.join(td, "test.db")
            init_db(db_path).close()
            results = query_similar("anything at all", top_k=3, db_path=db_path)
            assert results == []
    ...
    def test_query_short_tokens_returns_empty(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            db_path = os.path.join(td, "test.db")
            write_memory("US-1", "test", "approach", "pass", 1, db_path)
            results = query_similar("ab cd", top_k=3, db_path=db_path)
            assert results == []
