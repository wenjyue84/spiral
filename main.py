...
Subcommands:
  ...
  extract-failed-stories  Generate triage report from results.tsv for stuck stories (US-613)
...
def cmd_extract_failed_stories(args: argparse.Namespace) -> None:
    """Generate triage report from results.tsv for stuck stories (US-613).

    Usage: spiral extract-failed-stories [--min-retries N] [--format json|markdown]
    """
    sys.path.insert(0, str(Path(__file__).parent / "lib"))
    from extract_failed_stories import (  # type: ignore[import-untyped]
        extract_failed_stories,
        format_markdown,
    )

    results_path = getattr(args, "results", "results.tsv")
    min_retries = getattr(args, "min_retries", 1)
    output_format = getattr(args, "output_format", "json")

    report = extract_failed_stories(results_path, min_retries=min_retries)
...
def main():
    ...
    extract_failed_parser = subparsers.add_parser(
        ...
        help="Generate triage report from results.tsv for stuck stories (US-613)",
    )
    extract_failed_parser.add_argument(
        ...
        default=1,
        metavar="N",
        help="Minimum retry count to include in report (default: 1)",
    )
    extract_failed_parser.add_argument(
        "--format",
        dest="output_format",
