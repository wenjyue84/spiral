...
function spiralApiPlugin() {
  ...
  return {
    ...
    configureServer(server: { middlewares: { use: (path: string, fn: (req: IncomingMessage, res: ServerResponse, next: () => void) => void) => void } }) {
      ...
      // Ports all compute functions from lib/ui/spiral_dashboard.py into TypeScript.
      // Returns: overview, velocity, modelPerformance, retryAnalysis, resourceUsage,
      //   bottlenecks, iterationVelocity, statusBreakdown, tokenForecast, qualityScores,
      //   epics, decomposition, failureReasons, insights, latestScreenshot
      server.middlewares.use('/api/analytics', (req: IncomingMessage, res: ServerResponse, next: () => void) => {
        ...
        try {
          ...
          // Categorize: cost_ceiling, dependency_blocked, too_large, rejected, never_attempted, pending_retry
          ...
          const categorize = (s: PrdStory): FailureCategory => {
            ...
            // No explicit failure reason — infer from state
          ...
          // Include ALL non-passing stories (failed + pending) for full visibility
          ...
          // Sort: cost_ceiling first, then dependency_blocked, too_large, rejected, pending_retry, never_attempted
          .sort((a, b) => {
